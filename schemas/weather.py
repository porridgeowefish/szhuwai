...
class WeatherSummary(BaseModel):
    ...
    points_weather: List[Dict] = Field(default_factory=list)  # 各点天气
    summary: WeatherSummaryInfo = Field(default_factory=WeatherSummaryInfo, description="汇总信息")
...
class WeatherCloudSeaAnalysis(BaseModel):
    ...
    @classmethod
    def set_level_by_score(cls, data: dict) -> dict:
        """根据评分自动设置云海等级"""
...
def analyze_weather_safety(weather: CityWeatherResponse) -> Dict:
    """分析天气安全性"""
    safety_issues = []
